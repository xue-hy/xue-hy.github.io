...
if (pathName === '/') {
    $(document).ready(function() {
        if ($("#bber-talk").length > 0) {
            var jsonUrl = ''
            jsonUrl = "https://6262-bber-4gcsdstn24ad8ae9-1302837542.tcb.qcloud.la/json/bber.json"
            $.getJSON(jsonUrl + "?t=" + Date.parse(new Date()), function(res) {
                ...
                $.each(res.data, function(i, item) {
                    ...
                    bberHtml += '<li class="item item-' + (i + 1) + '">' + '<a href="https://blog.hiyoung.fun/bb/" style="color: var(--font-color);/*font-weight: normal;*/">' + newdataTime + '： ' + urlToLink(item.content) + '</a></li>'
                });
